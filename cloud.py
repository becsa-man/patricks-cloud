...
Jamie Encode
 to the group.
 — Today at 6:01 PM
Patrick🌾 — Today at 6:02 PM
filtering by last letter of airtable recordid is super inconsistent and dogshit, so now the trigger conditions are 'Run J' for jamie, 'Run R' for rebecca =)
there should be 2 parts of the code that need to be updated, u can just control +f 'Run' when editing and u will see what needs to be updated :)))
the first step will upload them with an even distribution across each trigger condition anyway
or to save u time, this is jamies cloud.py file to paste in
import requests
import re
...
message.txt
8 KB
Patrick🌾 — Today at 6:09 PM
And this is rebeccas:
import requests
import re
from requests.adapters import HTTPAdapter
from requests.packages.urllib3.util.retry import Retry
...
def get_airtable_records(api_key, base_id, table_name):
    url = f'https://api.airtable.com/v0/{base_id}/{table_name}?filterByFormula=AND(NOT({{GitHub}} = ""), {{Status}} = "Run R")'
    headers = {
...
def main():
    ...
    for record in records:
        ...
        if github_url and status == 'Run R':
